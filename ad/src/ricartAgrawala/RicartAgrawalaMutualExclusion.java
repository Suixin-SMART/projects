...
public class RicartAgrawalaMutualExclusion extends Algorithm {
    ...
    public void init() {
        ...
        for (int i = 0; i < nbNeighbors; i++) {
            ...
            sendTo(i, message);
            log.info(procId + " : Envoi END_REGISTER a " + i);
        }
    ...
    synchronized void askForCritical() {
        log.info(procId + " : Demande de Section Critique");
        R = true;
        HSC = H + 1;
        nREL = totalProcessus;
        SyncMessage message;
    ...
    synchronized void receiveREQ(int procAuteur, int procTarget, int H_P, int door) {
        if (procTarget == procId) {
            log.info(procId + " : Recu REQ(" + H_P + ") de " + procAuteur);
            if (H < H_P) {
                H = H_P;
            }
            H++;
            if (R && ((HSC < H_P) || ((HSC == H_P) && procTarget < procAuteur))) {
                ...
                log.info(procId + " : Envoi REL a " + procAuteur);
            }
        } else {
            log.info(procId + " : Recu REQ(" + H_P + ") de " + procAuteur + " pour " + procTarget);
            SyncMessage message = new SyncMessage(MsgType.REQ, procAuteur, procTarget, H_P);
            sendTo(routeur[procTarget], message);
            log.info(procId + " : Envoi REQ(" + H_P + ") a " + procTarget + " par " + routeur[procTarget]);
        }
    }
    ...
    synchronized void receiveREL(int procAuteur, int procTarget, int door)
    {
        ...
        if (procTarget == procId)
        {
            log.info(procId + " : Recu REL de " + procTarget);
            nREL--;
            this.notify();
        }
        else
        {
            log.info(procId + " : Recu REL de " + procTarget + " pour " + procTarget);
            SyncMessage message = new SyncMessage(MsgType.REL, procAuteur, procTarget);
            sendTo(routeur[procTarget], message);
            log.info(procId + " : Envoi REL a " + procAuteur + " par " + routeur[procTarget]);
        }
    }
            ...
            for (int i = 0; i < totalProcessus; i++) {
                if (procId != i) {
                    ...
                    if (tabRelDiffere[i]) {
                        ...
                        sendTo(routeur[i], tm);
                        log.info(procId + " : Envoi REL a " + i + " par " + routeur[i]);
                        tabRelDiffere[i] = false;
                    }
                }
            }
